...
namespace auik
{
    MenuBar::ItemRef &MenuBar::ItemRef::set_callback(acul::unique_function<void(ClickEvent &)> callback)
    {
        if (_owner && _element_id != 0u) _owner->ensure_item(_element_id).callback = std::move(callback);
        return *this;
    }

    MenuBar::GroupRef MenuBar::ItemRef::add_group(const acul::vector<acul::string> &items) const
    {
        if (!_owner || _element_id == 0u) return MenuBar::GroupRef{nullptr, {}};
        return _owner->add_group(_element_id, items);
    }

    MenuBar::GroupRef MenuBar::add_group(u32 element_id, const acul::vector<acul::string> &items)
    {
        return GroupRef{this, append_item_group(element_id, items)};
    }
    ...
    {
        if ((!(ctx.reason & DrawReasonBits::record) && !(ctx.reason & DrawReasonBits::invalidate)) && ctx.reason == DrawReasonBits::none)
        {
            const auto transition = detail::get_widget_style_selector_transition(id());
        ...
        if (parent() && parent()->get_rect().id.tag_id == AUIK_TAG_WINDOW)
        {
            ...
            if ((!(ctx.reason & DrawReasonBits::record) && !(ctx.reason & DrawReasonBits::invalidate)) && (ctx.reason & (DrawReasonBits::external | DrawReasonBits::transient)))
                draw_popups(ctx);
            return;
        }
        draw_popups(ctx);
    ...
    void MenuBar::layout_popup(u32 depth, const amal::rect &anchor, const acul::vector<u32> &ids)
    {
        ...
        for (u32 item_id : ids)
        {
            ...
            if (item->separator)
            {
                ...
                row->set_size({AUIK_F32_FILL, AUIK_F32_FIT});
            }

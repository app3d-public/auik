...
namespace auik
{
    namespace
    {
        ...
        static PostEffectNode *make_node(void *data, PFN_post_effect_record record, PFN_post_effect_update update,
                                         PFN_post_effect_destroy destroy,
                                         ...
                                         PFN_post_effect_update_batch update_batch = nullptr)
        {
            ...
            node->record = record;
            node->update = update;
            node->record_batch = record_batch;
            node->update_batch = update_batch;
            ...
            return make_node(
                ...
                [](void *effect_data, DrawStream *stream, DrawDataID *draw_ids, const void *draw_data, u32 count,
                   const void *) {
                    ...
                    const f32 alpha = static_cast<AlphaHandlerData *>(effect_data)->alpha;
                    for (auto &copy : copies) apply_alpha_to_quads(copy, alpha);
            ...
            return make_node(
                ...
                [](void *effect_data, DrawStream *stream, DrawDataID *draw_ids, const void *draw_data, u32 count,
                   const void *) {
                    ...
                    const f32 alpha = static_cast<AlphaHandlerData *>(effect_data)->alpha;
                    for (auto &copy : copies) apply_alpha_to_textures(copy, alpha);
                    stream->push_data_batch_to_stream(stream, copies.data(), count, draw_ids);
                },
                [](void *effect_data, DrawStream *stream, DrawDataID *draw_ids, const void *draw_data, u32 count,
                   const void *) {
                ...
                [](void *effect_data, DrawStream *stream, DrawDataID *draw_ids, const void *draw_data, u32 count,
                   const void *post_data) {
                    ...
                    if (handler && handler->runtime && fade_post && fade_post->id < handler->runtime->entries.size())
                    {
                        const auto &entry = handler->runtime->entries[fade_post->id];
                        if (entry.valid && entry.ref_count > 0u)
                            alpha = resolve_fade_alpha(entry.payload, handler->is_fade_in);
                    }
                    acul::vector<QuadsInstanceData> copies;
                    copies.resize(count);
                    std::memcpy(copies.data(), draw_data, sizeof(QuadsInstanceData) * count);
    ...
    PostEffectNode *create_post_effect_node(void *data, PFN_post_effect_record record, PFN_post_effect_update update,
                                            PFN_post_effect_destroy destroy, PFN_post_effect_record_batch record_batch,
                                            PFN_post_effect_update_batch update_batch)
    {
        return make_node(data, record, update, destroy, record_batch, update_batch);
    }
    ...
    PostFxChain *add_post_effect_to_widget(Widget *widget, PostEffect *effect, const void *instance_data,
                                           const void *post_data)
    {
        if (!widget || !effect) return nullptr;
        auto *chain = widget->add_post_effect(effect, instance_data, post_data);
        if (chain) invalidate_owner_widget(widget);
        return chain;
    }

    bool remove_post_effect_from_widget(Widget *widget, PostFxChain *chain)
    {
        if (!widget || !chain) return false;
        const bool removed = widget->remove_post_effect(chain);
        if (removed) invalidate_owner_widget(widget);
        return removed;
    }
} // namespace auik

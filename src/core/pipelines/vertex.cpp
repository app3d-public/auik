...
namespace auik::detail
{
    static void sync_frame_to_master(DrawStream *stream, StreamSyncState *state, Context &ctx, u32 frame_id)
    {
        ...
        ctx.gpu_ctx->vertex_stream.copy_stream_frame_data(stream, frame_id, state->master_id);
    ...
    static DrawDataID push_data_to_stream(DrawStream *stream, const void *data)
    {
        ...
        return gpu_ctx->vertex_stream.push_data_to_stream(stream, data, frame_id);
    }
    ...
    static void push_data_batch_to_stream(DrawStream *stream, const void *data, u32 count, DrawDataID *out_ids)
    {
        ...
        stream->flags |= StreamFlagBits::invalidate;
        ctx.dirty_flags |= DirtyFlagBits::streams;
        assert(gpu_ctx->vertex_stream.push_data_batch_to_stream &&
               "GPU vertex stream batch push dispatch is not initialized");
        gpu_ctx->vertex_stream.push_data_batch_to_stream(stream, data, count, out_ids, frame_id);
    }

    static void update_data_vertex_stream(DrawStream *stream, DrawDataID draw_data_id, const void *data)
    {
        auto &ctx = get_context();
        auto *state = static_cast<StreamSyncState *>(stream->runtime_data);
        ...
        stream->flags |= StreamFlagBits::invalidate;
        ctx.dirty_flags |= DirtyFlagBits::streams;
        ctx.gpu_ctx->vertex_stream.update_stream_data(stream, draw_data_id, data, ctx.frame_id);
    }

    static void invalidate_data_vertex_stream(DrawStream *stream, DrawDataID draw_data_id)
    {
        ...
        stream->flags |= StreamFlagBits::invalidate;
        ctx.dirty_flags |= DirtyFlagBits::streams;
        if (ctx.gpu_ctx->vertex_stream.invalidate_stream_data)
            ctx.gpu_ctx->vertex_stream.invalidate_stream_data(stream, draw_data_id, ctx.frame_id);
    }

    static void invalidate_data_batch_vertex_stream(DrawStream *stream, const DrawDataID *draw_data_ids, u32 count)
    {
        if (count == 0) return;
        ...
        stream->flags |= StreamFlagBits::invalidate;
        ctx.dirty_flags |= DirtyFlagBits::streams;
        if (!ctx.gpu_ctx->vertex_stream.invalidate_stream_data) return;
        for (u32 i = 0; i < count; ++i)
        {
            if (draw_data_ids[i].render_id == AUIK_INVALID_DRAW_DATA_ID) continue;
            ctx.gpu_ctx->vertex_stream.invalidate_stream_data(stream, draw_data_ids[i], ctx.frame_id);
        }
    }

    static void update_data_batch_vertex_stream(DrawStream *stream, const DrawDataID *draw_data_ids, const void *data,
                                                u32 count)
    {
        if (count == 0) return;
        ...
        stream->flags |= StreamFlagBits::invalidate;
        ctx.dirty_flags |= DirtyFlagBits::streams;
        assert(ctx.gpu_ctx->vertex_stream.update_stream_data_batch &&
               "GPU vertex stream batch update dispatch is not initialized");
        ctx.gpu_ctx->vertex_stream.update_stream_data_batch(stream, draw_data_ids, data, count, ctx.frame_id);
    }

    static void render_vertex_stream(DrawStream *stream, void *render_ctx, GPUContext *gpu_context)
    {
        const u32 frame_id = get_context().frame_id;
        if (stream->draw_sizes[frame_id] == 0) return;
        gpu_context->vertex_stream.render_stream(stream, render_ctx, gpu_context, frame_id);
    }
    ...
    static void clear_vertex_stream(DrawStream *stream, u32 frame_id)
    {
        auto &ctx = get_context();
        auto *state = static_cast<StreamSyncState *>(stream->runtime_data);
        ctx.gpu_ctx->vertex_stream.clear_stream(stream, frame_id);
    ...
    static void sync_vertex_stream(DrawStream *stream, u32 frame_id)
    {
    ...
    static void destroy_vertex_stream(DrawStream *stream)
    {
        auto *gpu_ctx = get_context().gpu_ctx;
        gpu_ctx->vertex_stream.destroy_stream_gpu_data(stream);
    ...
    static inline void setup_vertex_stream(DrawStream &stream)
    {
...
namespace auik
{
    void create_vertex_stream(DrawStream &stream)
    {
        detail::setup_vertex_stream(stream);
        stream.push_data_to_stream = &detail::push_data_to_stream;
        stream.push_data_batch_to_stream = &detail::push_data_batch_to_stream;
        stream.update_data_in_stream = &detail::update_data_vertex_stream;
        stream.update_data_batch_in_stream = &detail::update_data_batch_vertex_stream;
        stream.invalidate_data_in_stream = &detail::invalidate_data_vertex_stream;
        stream.invalidate_data_batch_in_stream = &detail::invalidate_data_batch_vertex_stream;
        stream.clear = &detail::clear_vertex_stream;
        stream.destroy = &detail::destroy_vertex_stream;

        auto &ctx = detail::get_context();
        auto *gpu_ctx = ctx.gpu_ctx;
        stream.stream_instances = gpu_ctx->vertex_stream.create_stream_gpu_data(ctx.frames_in_flight, gpu_ctx);

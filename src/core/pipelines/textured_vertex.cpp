...
namespace auik::detail
{
    ...
    static void invalidate_data_batch_textured_vertex_stream(DrawStream *stream, const DrawDataID *draw_data_ids,
                                                             u32 count)
    {
        if (count == 0) return;
        ...
        stream->flags |= StreamFlagBits::invalidate;
        ctx.dirty_flags |= DirtyFlagBits::streams;
        if (!ctx.gpu_ctx->textured_vertex_stream.invalidate_stream_data) return;
        for (u32 i = 0; i < count; ++i)
        {
            if (draw_data_ids[i].render_id == AUIK_INVALID_DRAW_DATA_ID) continue;
            ctx.gpu_ctx->textured_vertex_stream.invalidate_stream_data(stream, draw_data_ids[i], ctx.frame_id);
        }
    }
...
namespace auik
{
    void create_textured_vertex_stream(DrawStream &stream)
    {
        ...
        stream.invalidate_data_batch_in_stream = &detail::invalidate_data_batch_textured_vertex_stream;

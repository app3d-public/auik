...
#include <auik/v2/widgets/detail/table_base.hpp>
...
namespace auik::v2
{
    ...
    static bool is_auto_dock_size(f32 size) { return size == AUIK_F32_IGNORE || size <= 0.0f; }
    ...
    public:
        ...
        void draw_button(DrawCtx &ctx) { _button.draw(ctx, ctx.emit_hit_rect); }
    ...
    private:
        ...
        void ensure_menu()
        {
            ...
            if (auto *window = active_window(); window && window_popup_menu(window))
            {
                ...
                _menu = menu && menu->get_rect().id.tag_id == AUIK_TAG_POPUP_MENU ? static_cast<PopupMenu *>(menu)
                                                                                  : nullptr;
                if (menu && !_menu) { window->set_menu_widget(menu); }
                if (_menu)
                {
                    _menu_owner = window;
                    _menu->show();
        ...
        {
            if (!_open) return;
            const bool has_menu = rebuild_menu_model();
            if (!has_menu) return;
            ...
            _menu->set_popup_anchor_override(anchor);
            _menu->set_open(true);
        }
    ...
    {
        _nodes.push_back({});
        _nodes[0].settings.sizing = DockNodeSizing::stretch;
    ...
    void Dockspace::set_new_node_settings(DockNodeSettings settings) { _new_node_settings = settings; }
    ...
    {
        return !node.windows.empty() && !(node.settings.flags & DockspaceFlagBits::read_only);
    }
    ...
    {
        auto *node = get_node(node_id);
        if (!node || !window || (node->settings.flags & DockspaceFlagBits::read_only)) { return; }
        ...
        extracted->update_draw_commands(DrawReasonBits::full_redraw | DrawReasonBits::record);
        auto &ctx = detail::get_context();
        ctx.dirty_flags |= DirtyFlagBits::redraw | DirtyFlagBits::hit_rect_update;
        detail::mark_host_refresh_request();
    }
    ...
    {
        auto *node = get_node(node_id);
        if (!node || (node->settings.flags & DockspaceFlagBits::read_only)) return;
    ...
    void Dockspace::add_window(DockNodeID node, Window *window)
    {
        ...
        assert((n->settings.flags & DockspaceFlagBits::read_only) ||
               (window->window_flags & WindowFlagBits::decorated));
    ...
    bool Dockspace::dock_drag_window_to_tab_panel(Window *window, DockNodeID node_id)
    {
        ...
        if (node->settings.flags & DockspaceFlagBits::read_only) return false;
    ...
    void Dockspace::clear_node_chrome(Node &node)
    {
            ...
            can_invalidate_dock_chrome())
        {
            ...
            DrawCtx ctx{};
            ctx.emit_fn = &emit_draw_invalidate;
            ctx.emit_hit_rect = false;
            ctx.reason = DrawReasonBits::layout;
            if (node.tab_panel_draw.render_id != AUIK_INVALID_DRAW_DATA_ID)
    ...
    void Dockspace::toggle_menu(DockNodeID node_id)
    {
        ...
        if (_open_menu_node == node_id) { close_menu(); }
        else
        {
            close_menu();
    ...
    {
        if (node.windows.empty() || (node.settings.flags & DockspaceFlagBits::read_only)) return false;
    ...
    void Dockspace::sync_node_menu(DockNodeID node_id, Node &node, const amal::rect &panel_bounds,
                                   const amal::vec4 &panel_padding)
    {
        ...
        const amal::vec2 button_size = node.menu->required_size();
        const f32 panel_inner_h = amal::max(panel_bounds.size.y - panel_padding.y - panel_padding.w, 0.0f);
        const f32 button_x = panel_bounds.offset.x + panel_bounds.size.x - panel_padding.z - button_size.x;
        const f32 button_y =
            panel_bounds.offset.y + panel_padding.y + amal::max((panel_inner_h - button_size.y) * 0.5f, 0.0f);
        node.menu->layout_button({{button_x, button_y}, button_size}, clip_id());
    }
    ...
    amal::vec2 Dockspace::measure_node(DockNodeID node_id)
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < node->windows.size(); ++i)
            {
                auto *window = node->windows[i];
                if (!window) continue;
                const bool active = (node->settings.flags & DockspaceFlagBits::read_only) || i == selected_index;
                if (!active) continue;
                window->update_layout_min_size();
                if (node->settings.sizing == DockNodeSizing::fixed && is_auto_dock_size(node->settings.size))
                {
                    if (node->parent != AUIK_DOCK_NODE_INVALID)
                    {
                        ...
                        if (parent_node)
                        {
                            const amal::axis fixed_axis = parent_node->axis;
                            set_axis_size(required, fixed_axis,
                                          amal::max(axis_size(required, fixed_axis),
                                                    axis_size(window->required_size(), fixed_axis)));
                        }
                    }
                    else required = amal::max(required, window->required_size());
                }
            }
            if (needs_node_tab_panel(*node) && node->tabbar)
            {
                ...
                required.x = amal::max(required.x, node->tabbar->required_size().x + menu_w + padding.x + padding.z);
                required.y += amal::max(node->tabbar->required_size().y, menu_h) + padding.y + padding.w;
            }
        }

        if (node_id != 0u && node->settings.sizing == DockNodeSizing::fixed)
        {
            if (node->parent != AUIK_DOCK_NODE_INVALID)
            {
                const auto *parent_node = get_node(node->parent);
                if (parent_node)
                {
                    if (parent_node->axis == amal::axis::x && !is_auto_dock_size(node->settings.size))
                        required.x = amal::max(required.x, node->settings.size);
                    else if (parent_node->axis == amal::axis::y && !is_auto_dock_size(node->settings.size))
                        required.y = amal::max(required.y, node->settings.size);
                }
            }
        }
    ...
    bool Dockspace::prepare_active_window(Node &node, bool force_record, bool record_pass)
    {
        ...
        if (!(node.settings.flags & DockspaceFlagBits::read_only) && node.active_window_index != selected_index)
        {
    ...
    void Dockspace::layout_node(DockNodeID node_id, const amal::rect &bounds)
    {
        ...
        if (node->children.empty())
        {
            if (needs_node_tab_panel(*node))
            {
                ...
                if (node->tabbar)
                {
                    ...
                    const f32 panel_h =
                        amal::min(node->bounds.size.y, amal::max(node->tabbar->required_size().y, menu_h) +
                    ...
                    sync_node_menu(node_id, *node, panel_bounds, panel_padding);
                    const f32 menu_reserved = has_node_menu(*node) && node->menu ? node->menu->required_size().x : 0.0f;
                    node->tabbar->set_position(
                        {panel_bounds.offset.x + panel_padding.x, panel_bounds.offset.y + panel_padding.y});
                    node->tabbar->set_layout_size(
                        {amal::max(panel_bounds.size.x - panel_padding.x - panel_padding.z - menu_reserved, 0.0f),
                         amal::max(panel_bounds.size.y - panel_padding.y - panel_padding.w, 0.0f)});
            ...
            for (size_t i = 0; i < node->windows.size(); ++i)
            {
                ...
                if (!window) continue;
                const bool active = (node->settings.flags & DockspaceFlagBits::read_only) || i == selected_index;
                if (active) layout_leaf_window(*node, window);
            }
        ...
        auto helper_traits = [&](size_t before_child, size_t after_child, bool leading, bool &visible,
            ...
            interactive = before && after && has_resize_helper_flag(before->settings.flags, node->axis) &&
                          ...
                          before->settings.sizing != DockNodeSizing::fixed &&
                          after->settings.sizing != DockNodeSizing::fixed;
        };
        ...
        auto fixed_axis_size = [axis = node->axis](const Node &child) {
            const f32 minimum = axis_size(child.settings.min_size, axis);
            if (!is_auto_dock_size(child.settings.size)) return amal::ceil(amal::max(minimum, child.settings.size));

            const f32 current_size = axis_size(child.bounds.size, axis);
            if (current_size > 0.0f) return amal::ceil(amal::max(minimum, current_size));

            const f32 auto_size = axis_size(child.required_size, axis);
            return amal::ceil(amal::max(minimum, auto_size));
        };
        auto child_axis_min_size = [axis = node->axis](const Node &child) {
            return amal::ceil(
                amal::max(axis_size(child.settings.min_size, axis), axis_size(child.required_size, axis)));
        };
        f32 fixed_sum = 0.0f;
        f32 stretch_weight = 0.0f;
        f32 stretch_min_sum = 0.0f;
        for (DockNodeID child_id : node->children)
        {
            auto *child = get_node(child_id);
            if (!child) continue;
            if (child->settings.sizing == DockNodeSizing::fixed) fixed_sum += fixed_axis_size(*child);
            else
            {
                stretch_weight += amal::max(child->settings.size, 0.0001f);
                stretch_min_sum += child_axis_min_size(*child);
            }
        }

        const f32 children_available = amal::max(main_available - helpers_sum, 0.0f);
        const f32 stretch_available = amal::max(children_available - fixed_sum, 0.0f);
        acul::vector<f32> child_main_sizes;
        child_main_sizes.resize(node->children.size());
        acul::vector<bool> stretch_active;
        stretch_active.resize(node->children.size());
        if (node->settings.stretch_distribution == DockStretchDistribution::min_size_biased)
        {
            const f32 stretch_extra = amal::max(stretch_available - stretch_min_sum, 0.0f);
            for (size_t i = 0; i < node->children.size(); ++i)
            {
                auto *child = get_node(node->children[i]);
                if (!child) continue;
                if (child->settings.sizing == DockNodeSizing::fixed) child_main_sizes[i] = fixed_axis_size(*child);
                else
                {
                    const f32 child_min = child_axis_min_size(*child);
                    const f32 weight = amal::max(child->settings.size, 0.0001f);
                    child_main_sizes[i] =
                        child_min + (stretch_weight > 0.0f ? stretch_extra * (weight / stretch_weight) : 0.0f);
                }
            }
        }
        else
        {
            f32 remaining_size = stretch_available;
            f32 remaining_weight = stretch_weight;
            for (size_t i = 0; i < node->children.size(); ++i)
            {
                auto *child = get_node(node->children[i]);
                if (!child) continue;
                if (child->settings.sizing == DockNodeSizing::fixed) child_main_sizes[i] = fixed_axis_size(*child);
                else stretch_active[i] = true;
            }

            bool changed = true;
            while (changed && remaining_weight > 0.0f)
            {
                changed = false;
                for (size_t i = 0; i < node->children.size(); ++i)
                {
                    if (!stretch_active[i]) continue;
                    auto *child = get_node(node->children[i]);
                    if (!child) continue;
                    const f32 child_min = child_axis_min_size(*child);
                    const f32 weight = amal::max(child->settings.size, 0.0001f);
                    const f32 candidate = remaining_size * (weight / remaining_weight);
                    if (candidate >= child_min) continue;
                    child_main_sizes[i] = child_min;
                    stretch_active[i] = false;
                    remaining_size -= child_min;
            ...
            for (size_t i = 0; i < node->children.size(); ++i)
            {
                ...
                const f32 weight = amal::max(child->settings.size, 0.0001f);
                child_main_sizes[i] =
                    remaining_weight > 0.0f ? amal::max(remaining_size, 0.0f) * (weight / remaining_weight) : 0.0f;
            }
        }
        ...
        for (size_t i = 0; i < node->children.size(); ++i)
        {
            ...
            if (i + 1u == node->children.size())
            {
                ...
                if (remaining_to_edge >= child_main) child_main = remaining_to_edge;
            }
    ...
    void Dockspace::handle_tabbar_changed(DockNodeID node_id)
    {
        ...
        auto &node = *node_ptr;
        if (node.settings.flags & DockspaceFlagBits::read_only) return;
    ...
    void Dockspace::draw_node_tab_panel(DrawCtx &ctx, DockNodeID node_id, Node &node)
    {
        ...
        const bool emit_panel_hit = dock_ctx && dock_ctx->drag_zones_enabled && dock_ctx->drag_window &&
                                    ...
                                    !(node.settings.flags & DockspaceFlagBits::read_only);
        ...
        detail::emit_table_service_hit_rect(ctx, node.tab_panel_draw, panel_hit, ctx.emit_hit_rect);

        DrawCtx tab_ctx = ctx;
        tab_ctx.emit_hit_rect = node.tabbar->is_hittable();
        node.tabbar->draw(tab_ctx);
    ...
    void Dockspace::draw_node_menu(DrawCtx &ctx, DockNodeID node_id, Node &node)
    {
        ...
        button_ctx.emit_hit_rect = node.menu->is_hittable();
    ...
    void Dockspace::draw_resize_helpers(DrawCtx &ctx)
    {
        ...
        for (size_t i = 0; i < _resize_helpers.size(); ++i)
        {
            ...
            if (i >= _resize_helper_count)
            {
                ...
                detail::emit_table_service_hit_rect(ctx, helper.hit_draw, helper.hit_rect, ctx.emit_hit_rect);
                continue;
            }
            if (!helper.visible && !helper.interactive)
            {
                ...
                hidden_hit.bounds.size = {0.0f, 0.0f};
                detail::emit_table_service_hit_rect(ctx, helper.hit_draw, hidden_hit, ctx.emit_hit_rect);
                continue;
            }
            ...
            if (helper.interactive)
            {
                ...
                detail::emit_table_service_hit_rect(ctx, helper.hit_draw, helper_hit, ctx.emit_hit_rect);
            }
        }
    }
    ...
    void Dockspace::draw_node(DrawCtx &ctx, DockNodeID node_id)
    {
        ...
        auto draw_window = [&](size_t i, Window *window, bool respect_visibility, bool force_record) {
            ...
            child_ctx.emit_hit_rect = window->is_hittable();
            window->draw(child_ctx);
        };
        if (node->settings.flags & DockspaceFlagBits::read_only)
        {
            for (size_t i = 0; i < node->windows.size(); ++i) draw_window(i, node->windows[i], true, false);
            return;
        }
        if (selected_index < node->windows.size())
        {
            const bool force_record = node->record_active_window && !ctx.is_invalidating();
    ...
    void Dockspace::draw(DrawCtx &ctx)
    {
        ...
        if (sync_active_windows(root_node(), ctx.is_recording())) sync_clip_rect_cache();
        draw_node(ctx, root_node());
        draw_resize_helpers(ctx);
        if (_deferred_menu_popup_draw && _deferred_menu_popup_draw->is_open())
        {
            // @needs-testing: Popup draw is currently forced to the tail of the dockspace draw pass. If more
            // overlapping translucent/rounded UI cases appear, revisit whether this needs a real ordered UI pass/OIT.
    ...
    void Dockspace::on_drag(const amal::vec2 &delta, KeyPressState state)
    {
        ...
        auto shrink_side = [&](size_t begin, size_t end, f32 amount) {
            f32 remaining = amount;
            while (remaining > 0.0f)
            {
                size_t best_index = static_cast<size_t>(-1);
                f32 best_slack = 0.0f;
                for (size_t i = begin; i < end && i < parent_node->children.size(); ++i)
                {
                    auto *child = get_node(parent_node->children[i]);
                    if (!child || child->settings.sizing == DockNodeSizing::fixed) continue;
                    const f32 slack = next_sizes[i] - child_min_size(i);
                    if (slack <= best_slack) continue;
                    best_slack = slack;
                    best_index = i;
                }
                if (best_index == static_cast<size_t>(-1)) break;
                const f32 consumed = amal::min(remaining, best_slack);
                next_sizes[best_index] -= consumed;
                remaining -= consumed;
            }
        ...
        {
            const f32 applied = shrink_side(helper->after_child, parent_node->children.size(), requested_delta);
            if (applied <= 0.0f) return;
            next_sizes[helper->before_child] += applied;
        }
        else
        {
            const f32 applied = shrink_side(0u, helper->before_child + 1u, -requested_delta);
            if (applied <= 0.0f) return;
            next_sizes[helper->after_child] += applied;
        }
        ...
        for (size_t i = 0; i < parent_node->children.size(); ++i)
        {
            ...
            if (parent_node->settings.stretch_distribution == DockStretchDistribution::min_size_biased)
                next_size = amal::max(next_size - child_min_size(i), 0.0001f);
            else next_size = amal::max(next_size, 0.0001f);

            if (child->settings.size == next_size) continue;
            child->settings.size = next_size;
            changed = true;
        }
        if (!changed) return;
        update_own_layout();
    ...
    void Dockspace::on_drop(ElementID drag_id, ElementID drop_id)
    {
        ...
        DockNodeSettings settings = _new_node_settings;

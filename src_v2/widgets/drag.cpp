...
#include <auik/v2/widgets/drag.hpp>
#include <cmath>
#include <type_traits>
#include <utility>
...
namespace auik::v2
{
    ...
    {
        template <typename T>
        static TextFlags drag_text_flags(const DragUnitResolver *resolver)
        {
        ...
        void Draggable<T>::on_focus(bool focused)
        {
            ...
            if (focused && _select_all_on_next_focus)
            {
                _select_all_on_next_focus = false;
                select_all_text();
            }
            if (!focused) _select_all_on_next_focus = true;
        }

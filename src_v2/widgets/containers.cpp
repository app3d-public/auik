...
namespace auik::v2
{
    ...
    static inline f32 resolve_scroll_content_layout_width(f32 viewport_width, const amal::vec2 &content_size,
                                                          bool need_scroll_x)
    {
        if (need_scroll_x) return amal::max(content_size.x, viewport_width);
    ...
    static void activate_scrollbar_thumb_style(Widget *owner, u32 thumb_tag_id)
    {
        ...
        const auto style_flags = owner->update_style();
                                        ...
                                        u32 track_style_tag, u32 thumb_style_tag)
    {
        ...
        scrollbar =
            acul::alloc<detail::Scrollbar>(id, track_tag, thumb_tag, parent, axis, track_style_tag, thumb_style_tag);
    }

    static inline u32 scrollbar_thumb_style_for_track(u32 track_style_tag)
    {
        if (track_style_tag == AUIK_TAG_SCROLLBAR_TRACK_INTERNAL) return AUIK_TAG_SCROLLBAR_THUMB_INTERNAL;
        return AUIK_TAG_SCROLLBAR_THUMB;
    }
    ...
    namespace detail
    {
        ...
        static inline f32 resolve_child_layout_width(Widget *child, f32 fallback_width, f32 available_width)
        {
            ...
            if (child->stretch_width()) return amal::max(available_width, 0.0f);
        ...
        static void layout_inline_row(const acul::vector<Widget *> &children,
                                      const acul::vector<ChildLayoutFlags> &layouts, size_t row_start, size_t row_end,
                                      const amal::vec2 &row_pos, f32 available_width, f32 inline_spacing_x,
                                      f32 &row_height)
        {
            ...
            for (size_t i = row_end; i > row_start; --i)
            {
                ...
                if (child->stretch_width() || !child->is_fixed()) child->set_layout_size({width, child->size().y});
        ...
        void layout_child_widgets(const acul::vector<Widget *> &children, const acul::vector<ChildLayoutFlags> &layouts,
                                  const amal::vec2 &content_pos, f32 available_width, f32 inline_spacing_x)
        {
            amal::vec2 cursor = content_pos;
            ...
            const auto flush_inline_row = [&](size_t row_end, amal::vec2 &cursor_ref, bool &active, size_t &start) {
                if (!active) return;
                f32 row_height = 0.0f;
                layout_inline_row(children, layouts, start, row_end, cursor_ref, available_width, inline_spacing_x,
                                  row_height);
            ...
            for (size_t i = 0; i < children.size(); ++i)
            {
                ...
                if (child->stretch_width() || !child->is_fixed())
                    child->set_layout_size({available_width, child->size().y});
    ...
    f32 Block::resolved_inline_spacing() const { return 0.0f; }

    void Block::clear_children()
    {
    ...
    static inline bool is_concrete_size(f32 value) { return value < AUIK_F32_AUTO_EDGE; }

    bool Block::has_explicit_width() const
    {
        return !is_size_ignored(_explicit_size.x) || _size_key.x != 0u || _size_vec_key != 0u;
    }

    bool Block::has_explicit_height() const
    {
        return !is_size_ignored(_explicit_size.y) || _size_key.y != 0u || _size_vec_key != 0u;
    }
    ...
    f32 Block::resolved_explicit_width() const
    {
        if (_size_vec_key != 0u)
        {
            ...
            if (value.x != AUIK_F32_IGNORE) return value.x;
        }
        if (_size_key.x != 0u)
        {
            const f32 value = get_theme()->get_var<f32>(_size_key.x);
            if (value != AUIK_F32_IGNORE) return value;
        }
    ...
    f32 Block::resolved_explicit_height() const
    {
        ...
        {
            const amal::vec2 value = get_theme()->get_var<amal::vec2>(_size_vec_key);
            if (value.y != AUIK_F32_IGNORE) return value.y;
        }
        if (_size_key.y != 0u)
        {
    ...
    void Block::update_size_fixed_flag()
    {
        const bool fixed_bounds_by_size =
            is_concrete_size(resolved_explicit_width()) && is_concrete_size(resolved_explicit_height());
        if (fixed_bounds_by_size) widget_flags |= WidgetFlagBits::fixed_bounds;
        else widget_flags &= ~WidgetFlagBits::fixed_bounds;
    }

    void Block::set_width(f32 value)
    {
        _explicit_size.x = (is_size_ignored(value) || is_size_stretch(value)) ? value : amal::max(value, 0.0f);
        _size_key.x = 0u;
        _size_vec_key = 0u;
        update_size_fixed_flag();
        Widget::set_size({_explicit_size.x, requested_size().y});
    }

    void Block::set_height(f32 value)
    {
        _explicit_size.y = (is_size_ignored(value) || is_size_stretch(value)) ? value : amal::max(value, 0.0f);
        _size_key.y = 0u;
        _size_vec_key = 0u;
        update_size_fixed_flag();
    ...
    {
        _explicit_size.x = (is_size_ignored(value.x) || is_size_stretch(value.x)) ? value.x : amal::max(value.x, 0.0f);
        _explicit_size.y = (is_size_ignored(value.y) || is_size_stretch(value.y)) ? value.y : amal::max(value.y, 0.0f);
        _size_key = {0u, 0u};
        _size_vec_key = 0u;
        update_size_fixed_flag();
    ...
    void Block::set_kv_width(u32 key)
    {
        _size_key.x = key;
        _size_vec_key = 0u;
        _explicit_size.x = AUIK_F32_IGNORE;
        update_size_fixed_flag();
    }
    ...
    void Block::set_kv_height(u32 key)
    {
        _size_key.y = key;
        _size_vec_key = 0u;
        _explicit_size.y = AUIK_F32_IGNORE;
        update_size_fixed_flag();
    }
    ...
    void Block::set_kv_size(u32 key)
    {
        _size_vec_key = key;
        _size_key = {0u, 0u};
        _explicit_size = AUIK_SIZE_IGNORE;
        update_size_fixed_flag();
    }
    ...
    {
        detail::layout_child_widgets(children, _child_layouts, content_pos, content_size.x, resolved_inline_spacing());
    }

    void Block::update_layout_min_size() { update_layout_min_size_with(zero_vec4(), zero_vec4()); }
    ...
    void Block::update_layout_min_size_with(const amal::vec4 &margin, const amal::vec4 &padding)
    {
        ...
        if (is_concrete_size(resolved_explicit_width())) required.x = amal::max(resolved_explicit_width(), 0.0f);
        if (is_concrete_size(resolved_explicit_height())) required.y = amal::max(resolved_explicit_height(), 0.0f);
    ...
    void Block::update_layout_with(bool min_size_known, const amal::vec4 &margin, const amal::vec4 &padding)
    {
        ...
        if (is_concrete_size(resolved_explicit_width())) inner_size.x = amal::max(resolved_explicit_width(), 0.0f);
        else if (inner_size.x <= 0.0f) inner_size.x = inner_required.x;
        if (is_concrete_size(resolved_explicit_height())) inner_size.y = amal::max(resolved_explicit_height(), 0.0f);
    ...
    void DrawBlock::set_scrollbars_enabled(bool x, bool y)
    {
        ...
        if (!x && _scrollbar_x) _scrollbar_x->set_visible(false);
        if (!y && _scrollbar_y) _scrollbar_y->set_visible(false);
    }
    ...
    void DrawBlock::set_style_tag(u32 tag_id)
    {
        ...
        if (tag_id != 0u) set_rect_tag_id(tag_id);
    }
    ...
    void DrawBlock::update_layout_min_size()
    {
        update_layout_min_size_with(draw_margin(), sum_padding(draw_padding(), _content_padding));
    }
    ...
    void DrawBlock::update_scroll_clip(const amal::vec2 &content_pos, const amal::vec2 &view_size)
    {
        const amal::vec4 own_clip = clip_id() != 0xFFFFu
                                        ...
                                        : (parent() ? parent()->get_content_clip_rect() : get_main_viewport());
    ...
    void DrawBlock::request_scroll_layout_update(DrawReasonFlags reason)
    {
        ...
        add_render_command<detail::ScrollEventTraits>(this, [this, reason]() {
            update_layout(true);
    ...
    void DrawBlock::update_layout(bool min_size_known)
    {
        ...
        amal::vec2 children_layout_size = compute_content_min_size();
        ...
        const amal::vec2 clip_pos{clip_ignores_padding_x ? position().x : content_pos.x,
                                  clip_ignores_padding_y ? position().y : content_pos.y};
        const amal::vec2 clip_view_size{
            clip_ignores_padding_x ? amal::max(visible_scroll_rect.z - (need_scroll_y ? bar_w : 0.0f), 0.0f)
                                   : _scroll_view_size.x,
            clip_ignores_padding_y ? amal::max(visible_scroll_rect.w - (need_scroll_x ? bar_h : 0.0f), 0.0f)
                                   : _scroll_view_size.y};
        update_scroll_clip(clip_pos, clip_view_size);
        ...
        f32 content_layout_width =
            resolve_scroll_content_layout_width(_scroll_view_size.x, _scroll_content_size, need_scroll_x);
        const amal::vec2 pre_layout_children_size = _scroll_content_size;
        layout_children(content_pos - _content_offset, {content_layout_width, _scroll_view_size.y});
        const amal::vec2 laid_out_children_size = detail::compute_children_layout_required_size(
            children, _child_layouts, resolved_inline_spacing(), content_layout_width, false);
        ...
        if (laid_out_scroll_content_size != pre_layout_children_size)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                const f32 layout_w =
                    resolve_scroll_content_layout_width(viewport_w, children_layout_size, refined_need_x);
                layout_children(content_pos - _content_offset, {layout_w, viewport_h});
                children_layout_size = detail::compute_children_layout_required_size(
        ...
        if (focus_parent() || needs_scroll_events) add_event_flags(EventFlagBits::click);
        else remove_event_flags(EventFlagBits::click);
        if (needs_scroll_events) add_event_flags(EventFlagBits::scroll | EventFlagBits::drag);
        else remove_event_flags(EventFlagBits::scroll | EventFlagBits::drag);
    ...
    void DrawBlock::translate(const amal::vec2 &delta)
    {
        ...
        {
            const auto clip = get_clip_rect(_content_clip_id);
            update_clip_rect(_content_clip_id, {clip.x + delta.x, clip.y + delta.y, clip.z, clip.w});
        }
    }
    ...
    void DrawBlock::draw(DrawCtx &ctx)
    {
        if (draw_style())
        {
            ...
            emit_quads_instance(ctx, quads_stream, _bg_draw_id, bg, _bg_rect, visible, ctx.emit_hit_rect);
        }
        Block::draw(ctx);
          ...
          _expanded(expanded)
    {
        ...
        _label = acul::alloc<Text>(AUIK_TAG_TEXT, label, amal::vec2{0.0f, 0.0f},
                                   WidgetFlagBits::visible | WidgetFlagBits::fixed_layout, this, AUIK_STYLE_TAG_NO_PAD,
    ...
    void CollapseHeader::update_depth(const amal::vec2 &depth_range)
    {
        ...
        if (_trigger) { _trigger->update_depth(header_content_range); }
        if (_label) { _label->update_depth(header_content_range); }
        for (auto *child : children)
        {
            if (!child) continue;
            child->update_depth(content_range);
        }
        _header_rect.depth = next_depth(header_bg_range);
        _header_rect.hit_depth = _header_rect.depth;
        _content_rect.depth = next_depth(content_range);

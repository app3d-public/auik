...
namespace auik::v2::detail
{
    ...
    static void update_data_textures_stream(DrawStream *stream, DrawDataID draw_data_id, const void *data)
    {
        auto &ctx = get_context();
        auto *state = static_cast<StreamSyncState *>(stream->runtime_data);
    ...
    static void invalidate_data_textures_stream(DrawStream *stream, DrawDataID draw_data_id)
    {
        ...
        stream->flags |= StreamFlagBits::invalidate;
        ctx.dirty_flags |= DirtyFlagBits::streams;
        if (ctx.gpu_ctx->textures.invalidate_stream_data)
    ...
    static void render_textures_stream(DrawStream *stream, void *render_ctx, GPUContext *gpu_context)
    {
    ...
    static void clear_textures_stream(DrawStream *stream, u32 frame_id)
    {
        auto &ctx = get_context();
        auto *state = static_cast<StreamSyncState *>(stream->runtime_data);
    ...
    static void sync_textures_stream(DrawStream *stream, u32 frame_id)
    {
    ...
    static void destroy_textures_stream(DrawStream *stream)
    {

...
namespace auik
{
    ...
    namespace detail
    {
        ...
        extern AUIK_EXPORT struct Context
        {
            ...
            Viewport *main_viewport = nullptr;
    ...
    inline Viewport *get_main_viewport() { return detail::get_context().main_viewport; }

    inline amal::vec4 get_main_viewport_rect()
    {
        auto *viewport = get_main_viewport();
        assert(viewport && "main viewport is not set");
        return {viewport->rect.offset.x, viewport->rect.offset.y, viewport->rect.size.x, viewport->rect.size.y};
    }

    inline Viewport *resolve_viewport(Viewport *viewport)
    {
        auto *main_viewport = get_main_viewport();
        assert((viewport || main_viewport) && "viewport is not set");
        return viewport ? viewport : main_viewport;
    }

    inline amal::vec4 get_viewport_rect(Viewport *viewport)
    {
        auto *resolved = resolve_viewport(viewport);
        return {resolved->rect.offset.x, resolved->rect.offset.y, resolved->rect.size.x, resolved->rect.size.y};
    }

    AUIK_EXPORT amal::vec4 get_widget_viewport_rect(const Widget *widget);
